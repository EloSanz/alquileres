...
import YonaContractFormSection from './YonaContractFormSection';
import YonaContractPreview from './YonaContractPreview';
...
export interface YonaContractEditorProps {
    open: boolean;
    draft?: any | null; // Temporarily any or define a specific YonaDraft if needed, or keep partial compatibility
    onClose: () => void;
}

export default function YonaContractEditor({
    open,
    draft,
    onClose
    ...
    useEffect(() => {
        if (open) {
            let initialData: YonaContractData;
            if (draft) {
                initialData = draft.data;
            } else {
                initialData = { ...defaultYonaContractData };
            }
            ...
            if (!initialData.arrendador_dni?.trim()) missing.push('DNI/RUC del Arrendador');
            ...
            if (!initialData.stand_numero?.trim()) missing.push('Número de Estacionamiento');
    ...
    const handleDataChange = useCallback((field: keyof YonaContractData, value: string) => {
        ...
        setContractData(prev => {
            ...
            if (!newData.arrendador_dni?.trim()) missing.push('DNI/RUC del Arrendador');
            ...
            if (!newData.stand_numero?.trim()) missing.push('Número de Estacionamiento');
        ...
        switch (field) {
            case 'arrendador_dni':
                if (value.trim() && !/^(\d{8}|\d{11})$/.test(value.trim())) {
    ...
    const validateFieldFormats = (): string[] => {
        ...
        // Validar DNI/RUC del arrendador (8 o 11 dígitos)
        if (contractData.arrendador_dni.trim() && !/^(\d{8}|\d{11})$/.test(contractData.arrendador_dni.trim())) {
            formatErrors.push('DNI/RUC del Arrendador debe tener 8 o 11 dígitos');
        }
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const exportRequiredFields = [
                ...
                { key: 'stand_numero', label: 'Número de Estacionamiento' }
            ];
                    ...
                    {splitView ? (
                        ...
                        <Box sx={{ height: '100%', display: 'flex', gap: 2, overflow: 'hidden', p: 0 }}>
                            ...
                            <Box sx={{
                                ...
                                >
                                    <YonaContractPreview data={contractData} fullPage />
                                </Box>
                            </Box>
                        </Box>
                    ) : (
